...
export const weeksAndDays = (start, end) => {
  ...
  for (let i = 0; i <= nbArrayWeeks(); i++) {
    currentTimeStampDate = nbStartDay === 7
    ...
    weeksList[`week${i}`] = {
      name: currentNbWeek < 10 ? `S0${currentNbWeek}` : `S${currentNbWeek}`,
  ...
  for (let i = 0; i <= nbArrayDays; i++) {
    ...
    daysList[`day${i + 1}`] = {
      ...
      borderRight: !!(i === weeksList.week0.colspan - 1 ||
...
export const couloirAndRowspan = (payload) => {
  ...
  for (let index = 0; index < payload.length; index++) {
    if (index === 0 || payload[index].id_couloir !== payload[index - 1].id_couloir) {
      arrayBase.push(true)
    } else {
  ...
  for (let indice = 0; indice < arrayBase.length; indice++) {
    if (arrayBase[indice]) {
      payload[indice] = { ...payload[indice], borderCellTop: true, displayCouloir: true, couloirRowspan: `${rowSpanCalc(arrayBase)[i]}` }
      payload[indice - 1] = { ...payload[indice - 1], borderCellBottom: true }
      i++
    } else {
...
export const coPlAndRowspan = (payload) => {
  ...
  for (let index = 0; index < newArray.length; index++) {
    if (
      index === 0 ||
      newArray[index].id_plateforme !== newArray[index - 1].id_plateforme ||
  ...
  for (let indice = 0; indice < arrayBase.length; indice++) {
    if (arrayBase[indice]) {
      payload[indice] = { ...payload[indice], borderAppTop: true, displayPlatform: true, platformRowspan: `${rowSpanCalc(arrayBase)[i]}` }
      payload[indice - 1] = { ...payload[indice - 1], borderAppBottom: true }
      i++
    } else {
      payload[indice] = { ...payload[indice] }
    }
