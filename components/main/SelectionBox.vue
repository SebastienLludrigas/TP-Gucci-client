  ...
  <section v-show="!displaySpinner" class="selection">
    ...
    <div class="selection-container">
      <div class="application-section">
        <div
          v-show="toggleAppChexboxesAll"
          class="application all"
        >
          <input
            type="checkbox"
  ...
  methods: {
    ...
    handleArrayFilter (event, cible = 'applications') {
      ...
      if (!event.target.checked) {
        ...
        newArrayContent = this.$store.getters.selectionApp.filter(item => (cible === 'couloirs' ? item.id_couloir : item.id) !== +event.target.id)
        ...
        this.$store.commit('setPartialSelectionApp', newArrayContent.filter((_, index) => index < 50))
        ...
        if (newArrayContent.length !== this.$store.getters.permanentAppPlCo.length) {
        ...
        if (this.$store.getters.selectionApp.length === 0) {
          ...
          if (cible === 'applications') {
            itemToAdd = this.$store.getters.permanentAppPlCo.filter(item => item.id === +event.target.id)
            ...
            itemToAdd = this.$store.getters.permanentAppPlCo.filter(item => item.id_couloir === +event.target.id)
          ...
          this.$store.getters.selectionApp.forEach(item => idsApp.push(item.id))
          this.$store.getters.selectionApp.forEach(item => idsCou.push(item.id_couloir))
          ...
          itemToAdd = this.$store.getters.permanentAppPlCo.filter((item) => {
            ...
            if (cible === 'couloirs') {
              ...
              if (this.allAppChecked) {
                ...
                if (item.id_couloir === +event.target.id || (idsCou.includes(item.id_couloir) && idsApp.includes(item.id))) {
                  return true
                }
                ...
                if ((item.id_couloir === +event.target.id && idsApp.includes(item.id)) || (idsCou.includes(item.id_couloir) && idsApp.includes(item.id))) {
                  return true
                }
              }
              ...
              if (this.allAppChecked) {
                ...
                // présent dans le tableau courant
                if ((item.id === +event.target.id && idsCou.includes(item.id_couloir)) || (idsCou.includes(item.id_couloir) && idsApp.includes(item.id))) {
                  return true
                }
              } else {
                ...
                if (item.id === +event.target.id || (idsCou.includes(item.id_couloir) && idsApp.includes(item.id))) {
                  return true
                }
              }
            }
        ...
        // Ainsi que le nouveau contenu du tableau partiel
        this.$store.commit('setPartialSelectionApp', itemToAdd.filter((_, index) => index < 50))
        // On réinitialise le compteur de rechargement du scroll infini
        this.$store.commit('setNbReload', 1)
        // On passe le reset du scroll à true afin que la barre de scroll remonte en haut du tableau
        this.$store.commit('setResetScrollTop', true)
    ...
    handleListAppAndCouFilter (cible) {
      ...
      if (cible === 'couloirs') {
        ...
        this.$store.getters.selectionApp.forEach(item => ids.push(item.id_couloir))
        // On filtre la liste complète des couloirs pour ne garder que celui/ceux dont l'id est inclut
        // dans notre tableau d'ids
        newListContent = this.$store.getters.permanentCouloirs.filter(item => ids.includes(item.id_couloir))
        ...
        // dans notre tableau d'ids
        newListContent = this.$store.getters.permanentApplications.filter(item => ids.includes(item.id_application))
    ...
    handleReservationButton (currentArray) {
      if (currentArray.length === 0) {
        ...
        const isThereOnlyOneCou = currentArray => currentArray.every(
