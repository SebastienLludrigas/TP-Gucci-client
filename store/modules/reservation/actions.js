...
export default {
  ...
  async creationReservation (context) {
    ...
    const response = await fetch('http://localhost:3000/reservation/creation', {
      method: 'POST',
    ...
    if (!response.ok && response.status === 401) {
      ...
      setTimeout(() => {
        context.commit('reservation/setDisplayModalResaSummary', false, { root: true })
    ...
    if (!response.ok && response.status === 500) {
      context.commit('setErrorMessage', 'La création de la réservation a échoué, veuillez réessayer')
      context.commit('setCreateError', true)
      setTimeout(() => {
        context.commit('reservation/setDisplayModalResaSummary', false, { root: true })
        context.commit('setCreateError', false)
  ...
  async updateReservation (context, kindOfUpdate) {
    const token = context.rootGetters['auth/token']
    const idResa = context.rootGetters['reservation/currentReservationId']
    const data = context.getters.newValueForUpdate
    console.log(JSON.stringify(data), idResa)

    try {
      const response = await fetch(`http://localhost:3000/reservation/update/${idResa}`, {
        method: 'POST',
      ...
      if (!response.ok && response.status === 500) {
        ...
        context.commit(`setUpdate${kindOfUpdate}`, false)
        context.commit('setErrorMessage', 'La mise à jour a échoué, veuillez réessayer')
        context.commit('setUpdateError', true)
        setTimeout(() => {
          context.commit('setDisplayModalUpdate', false)
          context.commit('setUpdateError', false)
  ...
  async deleteOneReservedApplication (context) {
    ...
    const { idResa, idCouloir, idPlateforme, idApplication } = context.rootGetters['reservation/deleteInfos']
    ...
    try {
      ...
      if (!response.ok && response.status === 500) {
        context.commit('setErrorMessage', 'La suppression a échoué, veuillez réessayer')
        context.commit('setDeleteError', true)
        setTimeout(() => {
          context.commit('setDisplayModalDelete', false)
          context.commit('setDeleteError', false)
  ...
  async loadInfosResas (context) {
    const response = await fetch('http://localhost:3000/reservation/infos')
    const responseData = await response.json()
